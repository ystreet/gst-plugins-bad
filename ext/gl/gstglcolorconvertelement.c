...
static void _do_convert (GstGLContext * context,
    GstGLColorConvertElement * convert);
static gboolean _init_convert (GstGLColorConvertElement * convert);
static gboolean _init_convert_fbo (GstGLColorConvertElement * convert);

static gboolean _do_convert_draw (GstGLContext * context,
    GstGLColorConvertElement * convert);

static inline GstGLContext *
_get_context (GstGLColorConvertElement * convert)
{
  return GST_GL_BASE_FILTER (convert)->context;
}

#define GST_GL_COLOR_CONVERT_ELEMENT_FORMATS "{ RGB, RGBx, RGBA, BGR, BGRx, BGRA, " \
                               "xRGB, xBGR, ARGB, ABGR, Y444, I420, YV12, " \
                               "Y42B, Y41B, NV12, NV21, YUY2, UYVY, AYUV, " \
                               "GRAY8, GRAY16_LE, GRAY16_BE, RGB16, BGR16 }"
static const gchar *convert_formats[] = {
  "RGB", "RGBx", "RGBA", "BGR", "BGRx", "BGRA", "xRGB", "xBGR", "ARGB", "ABGR",
  "Y444", "I420", "YV12", "Y42B", "Y41B", "NV12", "NV21", "YUY2", "UYVY",
  "AYUV", "GRAY8", "GRAY16_LE", "GRAY16_BE", "RGB16", "BGR16"
};
...
struct _GstGLColorConvertElementPrivate
{
  gboolean result;
...
static gboolean _gst_gl_color_convert_element_set_caps (GstBaseTransform * bt,
    GstCaps * in_caps, GstCaps * out_caps);
static GstCaps *_gst_gl_color_convert_element_transform_caps (GstBaseTransform *
    bt, GstPadDirection direction, GstCaps * caps, GstCaps * filter);
static void _gst_gl_color_convert_element_gl_stop (GstGLBaseFilter *
    base_filter);
static gboolean
_gst_gl_color_convert_element_propose_allocation (GstBaseTransform * bt,
    GstQuery * decide_query, GstQuery * query);
static gboolean _gst_gl_color_convert_element_get_unit_size (GstBaseTransform *
    trans, GstCaps * caps, gsize * size);
static GstFlowReturn
_gst_gl_color_convert_element_prepare_output_buffer (GstBaseTransform * bt,
    GstBuffer * inbuf, GstBuffer ** outbuf);
static GstFlowReturn _gst_gl_color_convert_element_transform (GstBaseTransform *
...
GST_STATIC_PAD_TEMPLATE ("src",
    ...
    GST_STATIC_CAPS (GST_VIDEO_CAPS_MAKE_WITH_FEATURES
        (GST_CAPS_FEATURE_MEMORY_GL_MEMORY,
            GST_GL_COLOR_CONVERT_ELEMENT_FORMATS)));
...
gst_gl_color_convert_element_class_init (GstGLColorConvertElementClass * klass)
{
  ...
  g_type_class_add_private (klass, sizeof (GstGLColorConvertElementPrivate));

  base_filter->gl_stop = _gst_gl_color_convert_element_gl_stop;

  bt_class->transform_caps = _gst_gl_color_convert_element_transform_caps;
  bt_class->set_caps = _gst_gl_color_convert_element_set_caps;
  bt_class->propose_allocation =
      _gst_gl_color_convert_element_propose_allocation;
  bt_class->get_unit_size = _gst_gl_color_convert_element_get_unit_size;
...
{
  convert->priv = GST_GL_COLOR_CONVERT_ELEMENT_GET_PRIVATE (convert);
...
static void
_gst_gl_color_convert_element_gl_stop (GstGLBaseFilter * base_filter)
{
  GstGLColorConvertElement *convert =
      GST_GL_COLOR_CONVERT_ELEMENT (base_filter);
  const GstGLFuncs *gl = _get_context (convert)->gl_vtable;
  guint i;

  if (convert->fbo || convert->depth_buffer) {
    gst_gl_context_del_fbo (_get_context (convert), convert->fbo,
...
static gboolean
_gst_gl_color_convert_element_set_caps (GstBaseTransform * bt,
    GstCaps * in_caps, GstCaps * out_caps)
{
...
static GstCaps *
_gst_gl_color_convert_element_transform_caps (GstBaseTransform * bt,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter)
{
  ...
  for (i = 0; i < n; i++) {
    GstStructure *s = gst_caps_get_structure (result, i);
    GValue list = G_VALUE_INIT;
    GValue fmt = G_VALUE_INIT;
    gint j;

    gst_caps_set_features (result, i,
        gst_caps_features_from_string (GST_CAPS_FEATURE_MEMORY_GL_MEMORY));
    gst_structure_remove_fields (s, "colorimetry", "chroma-site", NULL);

    g_value_init (&list, GST_TYPE_LIST);

    for (j = 0; j < G_N_ELEMENTS (convert_formats); j++) {
      g_value_init (&fmt, G_TYPE_STRING);
      g_value_set_string (&fmt, convert_formats[j]);
    ...
    GstQuery * decide_query, GstQuery * query)
{
  ...
  if (pool == NULL && need_pool) {
    ...
    if (decide_query) {
      ...
      decide_pool = gst_base_transform_get_buffer_pool (bt);
    }
    ...
    if (decide_pool && GST_IS_GL_BUFFER_POOL (decide_pool)
      ...
      GST_DEBUG_OBJECT (convert, "create new pool");
      if (decide_pool)
        gst_object_unref (decide_pool);
      pool = gst_gl_buffer_pool_new (_get_context (convert));
...
no_caps:
  {
    GST_DEBUG_OBJECT (convert, "no caps specified");
    return FALSE;
  }
invalid_caps:
  {
    GST_DEBUG_OBJECT (convert, "invalid caps specified");
    return FALSE;
  }
config_failed:
  {
    GST_DEBUG_OBJECT (convert, "failed setting config");
    return FALSE;
  }
}

static gboolean
_gst_gl_color_convert_element_get_unit_size (GstBaseTransform * trans,
    ...
    GstBuffer * inbuf, GstBuffer ** outbuf)
{
  ...
  g_return_val_if_fail (convert->outbuf == NULL, GST_FLOW_ERROR);

  convert->inbuf = inbuf;

  gst_gl_context_thread_add (_get_context (convert),
...
static GstFlowReturn
_gst_gl_color_convert_element_transform (GstBaseTransform * bt,
    GstBuffer * inbuf, GstBuffer * outbuf)
{
  GstGLColorConvertElement *convert = GST_GL_COLOR_CONVERT_ELEMENT (bt);

  g_return_val_if_fail (outbuf == convert->outbuf, GST_FLOW_ERROR);
...
static void
_RGB_to_RGB (GstGLColorConvertElement * convert)
{
...
static void
_YUV_to_RGB (GstGLColorConvertElement * convert)
{
  struct ConvertInfo *info = &convert->priv->convert_info;
  GstVideoFormat out_format = GST_VIDEO_INFO_FORMAT (&convert->out_info);
  const gchar *out_format_str = gst_video_format_to_string (out_format);
...
#else
  gboolean texture_rg =
      gst_gl_context_check_feature (_get_context (convert), "GL_EXT_texture_rg")
      || gst_gl_context_check_feature (_get_context (convert),
      "GL_ARB_texture_rg");
#endif
  gboolean apple_ycbcr = gst_gl_context_check_feature (_get_context (convert),
      "GL_APPLE_ycbcr_422");
  gboolean in_tex_rectangular = FALSE;

#if GST_GL_HAVE_OPENGL
  GstMemory *memory = gst_buffer_peek_memory (convert->inbuf, 0);
  if (gst_is_gl_memory (memory) && (USING_OPENGL (_get_context (convert))
          || USING_OPENGL3 (_get_context (convert)))) {
    in_tex_rectangular =
...
static void
_RGB_to_YUV (GstGLColorConvertElement * convert)
{
  struct ConvertInfo *info = &convert->priv->convert_info;
  GstVideoFormat in_format = GST_VIDEO_INFO_FORMAT (&convert->in_info);
  const gchar *in_format_str = gst_video_format_to_string (in_format);
...
static void
_RGB_to_GRAY (GstGLColorConvertElement * convert)
{
  struct ConvertInfo *info = &convert->priv->convert_info;
  GstVideoFormat in_format = GST_VIDEO_INFO_FORMAT (&convert->in_info);
  const gchar *in_format_str = gst_video_format_to_string (in_format);
...
static void
_GRAY_to_RGB (GstGLColorConvertElement * convert)
{
  struct ConvertInfo *info = &convert->priv->convert_info;
  GstVideoFormat out_format = GST_VIDEO_INFO_FORMAT (&convert->out_info);
  const gchar *out_format_str = gst_video_format_to_string (out_format);
...
static void
_bind_buffer (GstGLColorConvertElement * convert)
{
  const GstGLFuncs *gl = _get_context (convert)->gl_vtable;
...
static void
_unbind_buffer (GstGLColorConvertElement * convert)
{
  const GstGLFuncs *gl = _get_context (convert)->gl_vtable;
...
static gboolean
_init_convert (GstGLColorConvertElement * convert)
{
  GstGLFuncs *gl;
  gboolean res;
  struct ConvertInfo *info = &convert->priv->convert_info;
  ...
  gl = _get_context (convert)->gl_vtable;
  ...
  if (!gl->CreateProgramObject && !gl->CreateProgram) {
    gst_gl_context_set_error (_get_context (convert),
  ...
  if (info->out_n_textures > 1 && (!gl->DrawBuffers ||
          USING_GLES2 (_get_context (convert)))) {
  ...
  if (USING_GLES2 (_get_context (convert)) &&
  ...
  res =
      gst_gl_context_gen_shader (_get_context (convert), text_vertex_shader,
  ...
  gst_gl_context_clear_shader (_get_context (convert));
...
static gboolean
_init_convert_fbo (GstGLColorConvertElement * convert)
{
  GstGLFuncs *gl;
  ...
  if (USING_OPENGL (_get_context (convert))
      || USING_OPENGL3 (_get_context (convert))) {
    gl->RenderbufferStorage (GL_RENDERBUFFER, GL_DEPTH_COMPONENT, out_width,
        out_height);
    gl->RenderbufferStorage (GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, out_width,
        out_height);
  }
  if (USING_GLES2 (_get_context (convert))) {
  ...
  if (!gst_gl_context_check_framebuffer_status (_get_context (convert))) {
    gst_gl_context_set_error (_get_context (convert),
...
void
_do_convert (GstGLContext * context, GstGLColorConvertElement * convert)
{
  ...
  if (!gst_gl_memory_setup_buffer (_get_context (convert), NULL,
          &convert->out_info, NULL, convert->outbuf)) {
    convert->priv->result = FALSE;
    return;
  }
...
out:
  for (j--; j >= 0; j--) {
    ...
    if (out_tex->tex_type == GST_VIDEO_GL_TEXTURE_TYPE_LUMINANCE
      ...
      if (!gst_memory_map ((GstMemory *) convert->priv->out_tex[j], &from_info,
              GST_MAP_READ | GST_MAP_GL)) {
        gst_gl_context_set_error (_get_context (convert), "Failed to map "
            "intermediate memory");
        res = FALSE;
        continue;
      }
  ...
  if (convert->outbuf) {
    GstGLSyncMeta *sync_meta =
        gst_buffer_add_gl_sync_meta (_get_context (convert), convert->outbuf);

    if (sync_meta)
      gst_gl_sync_meta_set_sync_point (sync_meta, _get_context (convert));
  }
...
static gboolean
_do_convert_draw (GstGLContext * context, GstGLColorConvertElement * convert)
{
  GstGLFuncs *gl;
  struct ConvertInfo *c_info = &convert->priv->convert_info;
  guint out_width, out_height;
